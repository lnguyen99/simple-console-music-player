 ...
 *  <p>
 *  The audio format uses a sampling rate of 44,100 (CD quality audio), 
 *  16-bit, monaural.
 *
 *  <p>
...
public final class StdAudio {
  ...
  private static void init() {
    try {
      // 44,100 samples per second, 16-bit audio, mono, 
      ...
      // it gets divided because we can't expect the buffered data 
      // to line up exactly with when the sound card decides to push
      // out its samples.
  ...
  public static void play(double duration, Pitch n, 
                          int octave, Accidental accidental) {
    
    if(n.equals(Pitch.R)) {
      // play no sound (but do delay) if the note is a rest
      StdAudio.play(note(0, duration, 0.5));
      //System.out.println("rest: no sound");
    } else {
  ...
  /**
   * Write an array of samples (between -1.0 and +1.0) to std audio.
   ...
   * return them as a double array with values between -1.0 and +1.0.
   */
  ...
  /* Helper for loop and play */
  private static AudioClip doClip(String filename) {
    URL url = null;
    try {
  ...
  public static void play(String filename) {
    AudioClip clip = doClip(filename);
    clip.play();
  }
  ...
  public static void save(String filename, double[] input) {
    ...
    AudioFormat format = new AudioFormat(SAMPLE_RATE, 16, 
                                         1, true, false);
  ...
  public static void main(String[] args) {
    ...
    for (int i = 0; i <= StdAudio.SAMPLE_RATE; i++) 
      StdAudio.play(0.5 * Math.sin(2*Math.PI * 
    ...
    // doesn't terminate until all the sound leaves the speaker.
    StdAudio.close(); 
